...
def traffic_monitor_callback(pkt):
    if IP in pkt:
        ...
        traffic.update({tuple(map(str, (src_ip, dst_ip, proto, src_mac, dst_mac, sport, dport, flag, attack))): length})

def add_n_in(df, scan_time):
    time = 1
    for(h1, h2, proto, src_mac, dst_mac, sport, dport, flag, attack), total in traffic.most_common():
        ...
        data = pd.DataFrame({'saddr': [h1], 'daddr' : [h2], 'proto' : [proto], 'src_mac' : [src_mac], 'dst_mac' : [dst_mac], 'sport' : [sport], 'dport':[dport], 'flag':[flag], 'N_IN_Conn_P_SrcIP':[temp_s], 'N_IN_Conn_P_DstIP':[temp_d], 'length' :[total] , 'time' : [time], 'datarate' : [total/time], 'category' : [attack]})
...
def do_sniff(ori_df, scan_time):
    hosts={}
    df = pd.DataFrame(columns=['saddr', 'daddr', 'proto', 'src_mac', 'dst_mac', 'sport', 'dport', 'flag', 'N_IN_Conn_P_SrcIP', 'N_IN_Conn_P_DstIP', 'length', 'time', 'datarate', 'category'])
    sniff(prn=traffic_monitor_callback, filter = 'ip', timeout=scan_time, store=False)
